...
def filter_all(search):
    raw =[
        db.session.query(Ride).filter(search).all()    
    ]
...
def joinride(ride_id):
    ...
    db.session.commit()
    driver_id = db.session.query(Ride.driver_id).filter_by(ride_id=ride_id).first()
    driver =db.session.query(User.email).filter_by(user_id=driver_id[0]).first()
